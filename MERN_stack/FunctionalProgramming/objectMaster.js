...
// const first = pokémon.filter(p => p.id %3 ==0);
// console.log(first);

// 2- an array of pokémon objects that are "fire" type;

// const second = pokémon.filter(p => p.types[0] == "fire" );
// console.log(second);
...
// const third = pokémon.filter(p => p.types[1] != undefined);
// console.log(third);


//4- an array with just the names of the pokémon

// const fourth = pokémon.map(p => p.name)
// console.log(fourth);


// 5- an array with just the names of pokémon with an id greater than 99 

// const fifth =pokémon.filter(p => p.id > 99).map(p => p.name);
// console.log(fifth);
...
// const sixith = pokémon.filter(p => p.types[0] =="poison" && p.types[1] == undefined ).map(p=>p.name);

// console.log(sixith);

// 7-  an array containing just the first type of all the pokémon whose second type is "flying"

// const seventh = pokémon.filter(p => p.types[1] =="flying").map(p => p.types[0]);
// console.log(seventh);


//8-  a count of the number of pokémon that are "normal" type 

// const justCount = pokémon.filter(p=> p.types[0] == "normal");
// console.log(justCount);
// console.log(justCount.length);